...
run_release <- function(opts = parse_release_args()) {
  ...
  system(sprintf("git tag -a %s -m \"%s\"", tag_name, tag_msg))
    ...
    steps:
      ...
      - name: Post PR comment
        env:
          GH_TOKEN: ${{ github.token }}
        run: |
          PREVIEW=$(cat changelog_preview.txt)
          COMMENT="### Changelog Preview

          These entries will be added to CHANGELOG.md when this PR is included in a release:

          \`\`\`
          $PREVIEW
          \`\`\`

          *Auto-generated by [generate_changelog.R](../blob/master/scripts/generate_changelog.R)*"

          # Update existing comment or create new one
          EXISTING=$(gh pr view ${{ github.event.pull_request.number }} --json comments -q '.comments[] | select(.body | startswith("### Changelog Preview")) | .id' | head -1)
          if [ -n "$EXISTING" ]; then
