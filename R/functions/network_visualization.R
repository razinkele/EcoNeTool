...
plotfw <- function(net, col = NULL, lab = NULL, size = NULL,
  ...
  # Input validation
  if (missing(net) || is.null(net)) {
    stop("Parameter 'net' is required and cannot be NULL", call. = FALSE)
  }
  if (!igraph::is_igraph(net)) {
    stop("net must be an igraph network object", call. = FALSE)
  }
  if (igraph::vcount(net) == 0) {
    stop("Network must have at least one vertex", call. = FALSE)
  }
  ...
  node_groups <- split(seq_len(n), ynod)
  xpos_by_group <- lapply(node_groups, function(idx) {
    l <- length(idx)
    if (l == 0) return(numeric(0))
    positions <- seq(-l, l, length.out = l)
    positions[order(tl[idx])]  # Order by exact TL within bin
  })
...
#' @param show_uncertainty Logical. If TRUE, visualize trait prediction uncertainty (default: FALSE)
#' @param uncertainty_data Data frame with confidence scores. Must have columns: species, overall_confidence
...
create_foodweb_visnetwork <- function(net,
  ...
  # =================================================================
  # UNCERTAINTY VISUALIZATION (if enabled)
  # =================================================================
  if (show_uncertainty && !is.null(uncertainty_data)) {
    ...
    if (exists("map_confidence_to_size")) {
      # Match species names to confidence data
      node_confidence <- sapply(V(net)$name, function(sp_name) {
        conf <- uncertainty_data$overall_confidence[uncertainty_data$species == sp_name]
        if (length(conf) > 0 && !is.na(conf[1])) {
          return(conf[1])
        } else {
      ...
      # Adjust node sizes based on confidence (higher confidence = larger)
      confidence_multiplier <- map_confidence_to_size(node_confidence, base_size = 0.8, scale_factor = 0.4)
      nodes$value <- nodes$value * confidence_multiplier

      # Calculate border widths (lower confidence = thicker border)
      border_widths <- map_confidence_to_border(node_confidence, min_width = 1, max_width = 5)

      # Store for later use with color
      nodes$borderWidth <- border_widths
      nodes$confidence <- node_confidence
    }
  }
  ...
  if (show_uncertainty && !is.null(nodes$borderWidth)) {
    # With uncertainty visualization: thicker borders for uncertain nodes
    nodes$color <- I(lapply(1:nrow(nodes), function(i) {
      list(
        background = info$colfg[i],
        border = info$colfg[i],
        borderWidth = nodes$borderWidth[i],
        highlight = list(background = info$colfg[i], border = "black")
      )
    }))
  } else {
    # Standard visualization without uncertainty
  ...
  nodes$title <- sapply(1:vcount(net), function(i) {
    ...
    if (show_uncertainty && !is.null(nodes$confidence)) {
      conf_pct <- round(nodes$confidence[i] * 100, 1)
      conf_cat <- if (nodes$confidence[i] >= 0.7) {
        "High"
      } else if (nodes$confidence[i] >= 0.5) {
        "Medium"
      } else {
        "Low"
      }
      tooltip <- paste0(tooltip, "<br>",
                       "<b>Confidence: ", conf_pct, "% (", conf_cat, ")</b>")
    }
