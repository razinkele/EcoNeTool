...
lookup_fishbase_traits <- function(species_name, timeout = 5) {
  ...
  tryCatch({
    # Get species data with enforced timeout
    species_data <- tryCatch({
      with_timeout(rfishbase::species(species_name), timeout = timeout, on_timeout = NULL)
    }, error = function(e) {
    ...
    # Get morphology data (optional - with timeout)
    morph_data <- tryCatch({
      with_timeout(rfishbase::morphology(species_name), timeout = timeout, on_timeout = NULL)
    }, error = function(e) NULL)

    # Get ecology data (optional - with timeout)
    ecology_data <- tryCatch({
      with_timeout(rfishbase::ecology(species_name), timeout = timeout, on_timeout = NULL)
    }, error = function(e) NULL)
...
lookup_sealifebase_traits <- function(species_name, timeout = 10) {
  ...
  tryCatch({
    # Query SeaLifeBase using rfishbase with enforced timeout
    species_data <- tryCatch({
    ...
    # Try to get morphology data (optional - with timeout)
    morph_data <- tryCatch({
...
# Module-level cache for ontology data (read once, reuse across species lookups)
.ontology_cache <- new.env(parent = emptyenv())
.ontology_cache$data <- NULL
.ontology_cache$file <- NULL
...
lookup_ontology_traits <- function(species_name = NULL, aphia_id = NULL,
  ...
  tryCatch({
    # Read ontology traits database (cached after first read)
    if (is.null(.ontology_cache$data) || !identical(.ontology_cache$file, ontology_file)) {
      .ontology_cache$data <- read.csv(ontology_file, stringsAsFactors = FALSE)
      .ontology_cache$file <- ontology_file
    }
    ontology_data <- .ontology_cache$data
...
harmonize_fuzzy_habitat <- function(ontology_traits) {
  ...
  if (grepl("infauna|buried|sediment.interior|interstitial", modality)) {
    ep_class <- "EP1"

  # EP2: Epibenthic (on sediment surface, intertidal, benthic)
  } else if (grepl("epibenthic|epifauna|benthic|subtidal|intertidal|tidal|littoral|eulittoral", modality) &&
             !grepl("benthopel|demersal|near.bottom", modality)) {
    ep_class <- "EP2"

  # EP3: Benthopelagic (near bottom, demersal)
  } else if (grepl("benthopel|demersal|near.bottom", modality)) {
    ep_class <- "EP3"

  # EP4: Pelagic (water column)
  } else if (grepl("pelagic|water.column|planktonic|surface|midwater", modality)) {
    ep_class <- "EP4"
  }
...
lookup_species_traits <- function(species_name,
  ...
  if (!is.null(depth_min) || length(habitat_info) > 0) {
    ...
    ep_labels <- c("EP1"="Infaunal", "EP2"="Epibenthic", "EP3"="Benthopelagic", "EP4"="Pelagic")
  ...
  if (length(protection_info) > 0) {
    ...
    pr_labels <- c("PR0"="No protection", "PR2"="Tube/Burrow", "PR3"="Soft shell",
                    "PR5"="Chitin exoskeleton", "PR6"="Hard shell", "PR7"="Spiny", "PR8"="Armoured")
  ...
  # Attach raw_traits as attribute so callers can access DB-specific results
  # without needing to re-query (e.g., trait_research_server.R)
  attr(result, "raw_traits") <- raw_traits
